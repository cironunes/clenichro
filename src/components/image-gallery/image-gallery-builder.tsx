                      ...
                      <div className="flex flex-wrap gap-2">
                        <label className="flex items-center gap-2">
                          ...
                          <select
                            ...
                            onChange={(e) =>
                              onLayoutChange(
                                e.target.value as ImageGalleryLayout
                              )
                            }
                            className="rounded-md border border-input bg-background px-3 py-1 text-sm"
                            aria-label="Layout"
                          ...
                          >
                            <div className="flex items-center justify-between mb-4">
                              <div className="text-sm text-muted-foreground">
                                {state.context.selectedUploadIds.size > 0
                                  ? `${
                                      state.context.selectedUploadIds.size
                                    } image${
                                      state.context.selectedUploadIds.size > 1
                                        ? "s"
                                        : ""
                                    } selected`
                                  : "Select images to add"}
                            ...
                            <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-3 max-h-[500px] overflow-y-auto">
                              {state.context.uploadedPreviews.map((image) => {
                                const isSelected =
                                  state.context.selectedUploadIds.has(image.id);
                                return (
                                  <div
                                    key={image.id}
                                    className="relative group cursor-pointer"
                                    onClick={() =>
                                      handleToggleUploadSelection(image.id)
                                    }
                                  >
                                    <div
                                      className={cn(
                            ...
                            <div className="border rounded-lg p-4 bg-background">
                              <div className="flex items-center justify-between mb-4">
                                <div className="text-sm text-muted-foreground">
                                  {state.context.selectedImageIds.size > 0
                                    ? `${
                                        state.context.selectedImageIds.size
                                      } image${
                                        state.context.selectedImageIds.size > 1
                                          ? "s"
                                          : ""
                                      } selected`
                                    : "Select images to add"}
                              ...
                              <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-3 max-h-[500px] overflow-y-auto">
                                {state.context.unsplashResults.map((photo) => {
                                  const isSelected =
                                    state.context.selectedImageIds.has(
                                      photo.id
                                    );
                                  return (
                                    <div
                                      ...
                                      className="relative group cursor-pointer"
                                      onClick={() =>
                                        handleToggleImageSelection(photo.id)
                                      }
                                    >
                                      <div
                                        className={cn(
